...
export const Audio = {
    ...
    // ── Active AbortController for in-flight ElevenLabs fetch ────────────
    // Created fresh in speak() and aborted in stopAll().
    // This is the PRIMARY cancellation source for window-1 exits
    // (user taps × while ElevenLabs fetch is in-flight).
    _activeAbortController: null,

    async stopAll() {
        // Abort any in-flight ElevenLabs fetch first — before _stopAll touches
        // WebAudio nodes — so the fetch rejects immediately and cannot produce
        // an orphan handle after we return.
    ...
    async speak(text, artwork, narrative = null) {
        ...
        if (key) {
            ...
            // Create a fresh AbortController for this fetch — stored so stopAll()
            // can cancel it immediately if the user exits during the request.
            const abortCtrl = new AbortController();
            this._activeAbortController = abortCtrl;
            try {
                ...
                if (this._activeAbortController === abortCtrl) {
                    this._activeAbortController = null;
                }
            }
        }
